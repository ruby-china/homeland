window.EmojiModalView = Backbone.View.extend({
  ...
  activeFirstPanel() {
    ...
    const firstGroupName = this.$el.find('.nav-tabs li a').first().data("group");
  ...
  addGroup(group) {
    ...
    if (group.name === 'favorites') {
      if (group.icons.length === 0) { return false; }
    }
    const navTab = `
  ...
  preview(e) {
    ...
    const code = `:${target.data('code')}: `;
    const html = `<img class='emoji' src='${this.findEmojiUrlByName(emojiName)}' /> ${code}`;
