...
module ApplicationHelper
  def sanitize_topic(body)
    sanitize body, :tags => %w(p br img h1 h2 h3 h4 blockquote pre code b i strike u a ul ol li), :attributes => %w(href src class title alt target rel)
  end

  def sanitize_reply(body)
    sanitize body, :tags => %w(p br img h1 h2 h3 h4 blockquote pre code b i strike u a ul ol li), :attributes => %w(href src class title alt target rel data-floor)
  end
