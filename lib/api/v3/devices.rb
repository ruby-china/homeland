  ...
  module V3
    class Devices < Grape::API
      resource :devices do
        ...
        params do
          requires :kind, type: String, values: %w(ios android)
          requires :token, type: String
        end
        post '' do
          doorkeeper_authorize!

          @device = current_user.devices.find_or_initialize_by(kind: params[:kind].downcase)
        ...
        delete '' do
          doorkeeper_authorize!
          current_user.devices.where(kind: params[:kind].downcase, token: params[:token]).delete_all
          { ok: 1 }
        end
      end
    end
