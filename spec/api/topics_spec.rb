  ...
  describe "GET /api/v3/topic/:id/replies.json" do
    ...
    context "has login" do
      it "should work" do
        ...
        expect(json["replies"][0]).to include("id", "user", "body_html", "created_at", "updated_at", "deleted")
        expect(json["replies"][0]["user"]).to include("id", "name", "login", "avatar_url")
        expect(json["replies"][0]["id"]).to eq r1.id
        expect(json["replies"][0]["abilities"]).to include("update", "destroy")
